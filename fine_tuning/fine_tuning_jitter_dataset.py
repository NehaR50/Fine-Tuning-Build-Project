...
if all([os.path.isfile(p) for p in all_dataset_paths]):
    ...
    val_df = jittered_titles_df[jittered_titles_df['seed_title'].isin(val_clean_titles)].copy()
    test_df = jittered_titles_df[jittered_titles_df['seed_title'].isin(test_clean_titles)].copy()
    train_df = jittered_titles_df[(~jittered_titles_df['seed_title'].isin(test_clean_titles)) & (~jittered_titles_df['seed_title'].isin(val_clean_titles))].copy()
...
# Base model
base_model_name = 'sentence-transformers/paraphrase-MiniLM-L6-v2'
...
def generate_training_examples(ds, take_longest_variant=True, infinite=True):
    ...
    if take_longest_variant:
        ds.loc[:, 'seed_title'] = ds['seed_title'].apply(get_clean_title)
